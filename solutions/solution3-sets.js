...
function union(objA,objB) {
    ...
    for (var key in objB) {  // ...all of objB
        union[key] = (union[key] || objB[key]);  //??why is this or?
    }
    return union;
}
...
function difference(objA,objB) {
    var diff = copy(objA);  //?? Why the need to copy object A?
    for (var key in objB) {  //?? How does the variable key know to reference the key in the object?
        delete diff[key];  //?? Why is copy of objA looping?
    }
    return diff;
}


// Optional: make functions available as module   //?? What is a module? 
