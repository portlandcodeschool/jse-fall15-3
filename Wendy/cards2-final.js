...
var cardTools = { 
		// //SOME DEFINITIONS
		// 	RANK is a number between 1 and 13
		// 	SUIT is a number between 1 and 4
		// 	cardID is a number between 1 and 51
		// 	COLOR is a string either red or black
		// 	NAME is a string with a number  (eg. 5 of Spades)
		//END OF DEFINITIONS//	
			
			suitNames :['', 'Hearts', 'Diamonds', 'Spades', 'Clubs'],
			rankNames :['', 'Ace', 'Two', 'Three','Four', 'Five', 'Six', 'Seven', 'Eight', 'Nine', 'Ten', 'Jack', 'Queen', 'King'],

			isValidDeck: function(num, low, high) {
				...
				if(num<low || num>high)
					return NaN;
				return true;
			}, //do not forget this pesky little comma between functions

			rank: function(cardID) { 
				return (this.isValidDeck(cardID, 0, 51)) && (Math.floor(cardID/4)+1);
			},

			suit: function(cardID) { //1---4, NaN
				return (this.isValidDeck(cardID, 0, 51)) && ((cardID%4)+1);
			},

			cardID: function(rank, suit) { //0,51, NaN //but HOW does the user know to add 5 arguements)
					return(this.isValidDeck(rank,1,13))&&(this.isValidDeck(suit,1,4))&&((rank-1)*4+(suit-1));
			},

			color: function(cardID) { //1,4  WHY aren't the perameters applying??????????????
				return (this.isValidDeck(cardID,0,51))&&((this.suit(cardID)<3)? 'red': 'black');	
			},

			name: function(cardID) {
				
				return (this.isValidDeck(cardID,0,51))&&(this.rankNames[(this.rank(cardID))]) + ' of ' +  (this.suitNames[(this.suit(cardID))]);
			}
	//This returns number 1 through 13//
